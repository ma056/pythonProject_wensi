...
def main(call_center, is_sync_history: bool = False):
    ...
    # print(len(contact_id_list_in_engserver_not_attr_get))
    # time.sleep(999)
    ...
    # for i in range(0, len(contact_id_list_in_engserver_not_search_get)):
    #     try:
    #         time_start = now()
    #         contact_id = contact_id_list_in_engserver_not_search_get[i]
    ...
    #         disconnect_timestamp = attr_json_dict.get('ctrDetails').get('DisconnectTimestamp').replace('T',' ').replace('Z','')
    ...
    #         time_diff = now() - time_start
    #         print(
    #             f"--- {i+1}/{len(contact_id_list_in_engserver_not_search_get)} -- {contact_id} updated -- {time_diff.total_seconds()}",
    #             end="\r")
    #     except Exception as e:
    #         print(
    #             f"!!! {contact_id_list_in_engserver_not_search_get[i]} insert error"
    #         )
    #         print(e)
...
def get_call_center_attr_1(call_center):
    eng_db = dbu.OneForma("RINGQA")
    sele_monitor = seleu.CallCenter(call_center)
    contact_id_list_in_engserver_not_search_get = eng_db.get_contact_ids_from_engserver_get_search()
    sele_monitor.browser_init_call_center(headless=False)
    cookies = sele_monitor.get_call_center_cookies()
    for i in range(0, len(contact_id_list_in_engserver_not_search_get)):
        try:
            contact_id = contact_id_list_in_engserver_not_search_get[i]
            ...
            eng_db.save_contact_search_result_to_engserver(
                contact_search_result)
            time.sleep(999)
            # contact_search_result = contact_search_result_list[i]
            # if "Contact ID" not in contact_search_result.keys():
            #     break
            # contact_id = contact_search_result["Contact ID"]
            # if contact_id in contact_id_list_in_engserver_not_search_get:
            #     eng_db.save_contact_search_result_to_engserver(
            #         contact_search_result)
            #     time_diff = now() - time_start
            #     print(
            #         f"--- {i+1}/{len(contact_search_result_list)} -- {contact_id} updated -- {time_diff.total_seconds()}",
            #         end="\r")
        except Exception as e:
            print(
                f"!!! {contact_id_list_in_engserver_not_search_get[i]['Contact ID']} insert error"
            )
            print(e)
    print("--- Search result saved")
...
def update_audio_length():
    eng_db = dbu.OneForma("RINGQA")
    file_operator = fileu.RingQAFileOpeartor("AU")
    all_file_list = file_operator.get_all_files_original()
    length_contact_ids = eng_db.get_contact_id_not_calculated_history()
    # length_contact_ids = ['527c2e4c-031b-48cd-b73f-e48c965885fc']
    print(f"--- Calcualte Audios {len(length_contact_ids)}")
