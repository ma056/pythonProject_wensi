...
def main(call_center, is_sync_history: bool = False):
    ...
    '''
    涉及表contact_info
    def get_contact_ids_from_engserver_call_center(self):
        select_sql = "select contact_id from call_center;"
    ...
    '''
    涉及表contact_info
    def save_new_contact_id_to_engserver_call_center(self, contact_id):
        insert_sql = f"insert into call_center (contact_id) values('{contact_id}');"
    ...
    '''
    涉及表contact_info
    def get_contact_ids_from_engserver_get_attr_call_center(self):
        ...
        return contact_ids
    '''
    # contact_id_list_in_engserver_not_attr_get = eng_db.get_contact_ids_from_engserver_get_attr_call_center()
    ...
    for i in range(0, len(contact_id_list_in_engserver_not_attr_get)):
        try:
            ...
            '''
            # def save_contact_id_attr_to_engserver_call_center(self, contact_id, contact_attr_dict,call_center):
            ...
            # '''
            # return self.execute_insert_sql_no_wait(insert_sql)
            # b_inserted = eng_db.save_contact_id_attr_to_engserver_call_center(contact_id, contact_attr_dict, call_center)
            b_inserted = eng_db.save_contact_id_attr_to_engserver(contact_id, contact_attr_dict, call_center)
    ...
    '''
    涉及表contact_info
    def get_contact_ids_from_engserver_get_search_call_center(self, call_center):
        ...
        return contact_ids
    '''
    # contact_id_list_in_engserver_not_search_get = eng_db.get_contact_ids_from_engserver_get_search_call_center(call_center)
...
def get_call_center_attr(sele_monitor, eng_db, contact_id_list, cookies, po):
    ...
    for i in range(0, len(contact_id_list)):
        try:
            ...
            涉及表contact_info
            '''
            # def save_contact_search_result_to_engserver_call_center(self, search_result_dict):
            #     if "Contact ID" not in search_result_dict.keys():
            #         return False
            #     else:
            #         contact_id = search_result_dict["Contact ID"]
            #         attr_system_number = search_result_dict['System phone number'].strip('+') if "System phone number" in search_result_dict else ""
            ...
            eng_db.save_contact_search_result_to_engserver(contact_search_result)
            time_diff = now() - time_start
